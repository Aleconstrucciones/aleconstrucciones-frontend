...
function ZeroAccidentSection({ data }: Props) {
  const heroImageUrl = data.securityZero.responsibleMedia?.url
    ? `${process.env.NEXT_PUBLIC_STRAPI_URL}${data.securityZero.responsibleMedia.url}`
    : null;
        ...
        <div className="grid md:grid-cols-2 gap-16 items-center">
          ...
          <div className="flex flex-col gap-6">
            <h2 className="text-4xl md:text-5xl font-bold">
              {data.securityZero.title}
            </h2>

            <p className="text-neutral-300 text-lg leading-relaxed">
              {data.securityZero.description}
            </p>

            <div className="mt-6">
              <p className="font-semibold text-white">
                {data.securityZero.responsibleName}
              </p>
              <p className="text-neutral-400 text-sm uppercase tracking-wide">
                {data.securityZero.responsibleRole}
              </p>
            </div>
          </div>
          ...
          {heroImageUrl && (
            <div className="relative w-full h-88 md:h-100">
              <Image
                src={heroImageUrl}
                alt={data.securityZero.responsibleMedia.alternativeText || data.securityZero.title}
                fill
